...
const Box = ({box_i_index,box_j_index}) => {
    ...
    const handleClick=(event)=>{
        ...
        let finalDesc=[];
        if(isVert){
            finalDesc.push(<div className='clickedLine' key={id} style={{color:'red'}}>{data.userName} connected dot {(i-1)*n+(j-1)+1} to dot {i*n+(j-1)+1}</div>);
        }
        else{
            finalDesc.push(<div className='clickedLine' key={id} style={{color:'red'}}>{data.userName} connected dot {(i-1)*n+(j-1)+1} to dot {(i-1)*n+j+1}</div>);
        }
